...
interface
uses
    uttsPortug, uttsPreproc, uttsProsodia, uttsUtils, mbrola, minireg, dvwav,
    windows, classes, sysutils, mmsystem, dialogs;
...
procedure liane_debug (nomeArq: string);

var
    liane_pitchRate: real;
    liane_durationRate: real;
    mbrola_dbname: string;
...
begin
    ...
    mbrola_dbName := getDLLPath_MBR+ '\br4';
    if not fileExists(mbrola_dbname) then
        mbrola_dbname := getDLLPath_MBR+'\lianetts\br4'
    else
    if not fileExists(mbrola_dbname) then
        mbrola_dbname := '\winvox\lianetts\br4';

    if init_MBR (pchar(mbrola_dbname)) < 0 then
	begin
            unload_MBR;
            exit;
	end;
...
var debug: textFile;
    i: integer;
begin
    assignFile (debug, nomeArqDebug);
    if FileExists(nomeArqDebug) then
        append (debug)
    else
        rewrite (debug);
...
begin
    ...
    for i := 0 to mbrolaCmd.count-1 do
        begin
            ...
            dif := dif + #$0d#$0a;
            if write_MBR (@dif[1]) < 0 then
                begin
                    close_MBR;
                    unload_MBR;
                    load_MBR;
                    init_MBR (pchar(mbrola_dbname));
                    liane_speak := false;
                end;
