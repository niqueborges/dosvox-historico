...
{       SQL FECHA
{       SQL EXECUTA "comando"
{       SQL PREPARA #1 "INSERT INTO nometabela (campo1, campo2, ... ) VALUES (?, ?, ?)" "SRR"
{       SQL LIBERA #1
{       SQL INJETA #1 $campo1, $campo2, ... [&]
{                     * o & é usado quando preparando comandos PEGA ou LISTA
{       SQL ULTIMOID x
{       SQL CHECA x
{       SQL PEGA #1 $name, $born, $died
{       SQL LISTA #1 lista separador
{       SQL EXIBE "comando sql" separador
{
{    Comandos de cópia e conversão
{
...
{
{    Ações com sprites
{        SPRITE CRIA arqsprite
{        SPRITE COLA arqsprite x y
{        SPRITE GRAVA arqsprite
{        SPRITE LIMPA
{
{    Ações com listas
{       seja l = []
...
interface
uses dvwin, dvcrt, windows, classes, synacode, graphics,
...
begin
    ...
    getdir (0, nomeDir);
    while sintFalando do;
    if executaProgEx (comando, nomeDir, param, SW_SHOWNORMAL ) >= 32 then
        begin
...
function processaSprite (lido: string): boolean;
...
function processaSQL (lido: string): boolean;
var
    primeiraLinha: boolean;
    nomeArq, comandoSQL: string;
    numPrep: integer;
    col: integer;
    diagnostico: string;
...
begin
    ...
    if copy (lido, 1, 1) = '"' then
         lido := 'EXIBE ' + lido;

    if maiuscAnsi (copy (lido, 1, 4)) = 'ABRE' then
        begin
            //SQL ABRE arquivo [&]
            ...
            try
                ...
                DB.Free;
                DB := NIL;
                sintWriteln ('Database não pode ser aberto usando ' + nomeArq);
                exit;
            end;
        end
    else

    if maiuscAnsi (copy (lido, 1, 5)) = 'FECHA' then
        begin
            //SQL FECHA
    ...
    if maiuscAnsi (copy (lido, 1, 7)) = 'EXECUTA' then
        begin
            ...
            if not extraiValor (lido, comandoSql) then exit;
            if lido <> '' then exit;
    ...
    if maiuscAnsi (copy (lido, 1, 7)) = 'PREPARA' then
        begin
            ...
            if not extraiNumArq (lido, np) then exit;
            numPrep := strToInt (np);
            if not extraiValor (lido, comandoSql) then exit;

            tiposPrep[numPrep] := '';
            if lido <> '' then
                if not extraiValor (lido, tiposPrep[numPrep]) then exit;
            if lido <> '' then exit;
    ...
    if maiuscAnsi (copy (lido, 1, 6)) = 'LIBERA' then
        begin
            //SQL LIBERA #1
    ...
    if maiuscAnsi (copy (lido, 1, 6)) = 'INJETA' then
        begin
            ...
            while lido <> '' do
                begin
                    if not extraiValor (lido, x) then break;
            ...
            if col <> length(tiposPrep[numPrep])+1 then
                begin
erro:               sintWriteln ('Número de parâmetros inconsistente');
                    exit;
                end;
    ...
    if maiuscAnsi (copy (lido, 1, 8)) = 'ULTIMOID' then
        begin
            //SQL ULTIMOID x
            ...
            if not extraiVariavel (lido, v, idx) then exit;
            guardaVar(v, idx, intToStr(DB.LastInsertRowID));
        end
    else
    ...
    if maiuscAnsi (copy (lido, 1, 5)) = 'CHECA' then
        begin
            ...
            if not extraiVariavel (lido, v, idx) then exit;
            guardaVar(v, idx, intToStr(ord(sqlErro)))
        end
    else

    if maiuscAnsi (copy (lido, 1, 4)) = 'PEGA' then
        begin
            ...
            if not sqlErro then
                begin
                    ...
                    while lido <> '' do
                        begin
                            if not extraiVariavel (lido, v, idx) then break;
                            if col > colsSel then goto erro;

                            case tiposSel[col] of
                                0:  guardaVar(v, idx, '');
                                1:  guardaVar(v, idx, intToStr(prep[numPrep].ColumnInt(col-1)));
                                2:  guardaVar(v, idx, FloatToStr(prep[numPrep].ColumnDouble(col-1)));
                                3:  guardaVar(v, idx, prep[numPrep].ColumnText(col-1));
                                4:  ;  // Blob
                            end;
    ...
    if maiuscAnsi (copy (lido, 1, 5)) = 'LISTA' then
        begin
            //SQL LISTA #1 lista separador
            ...
            nomeia := false;
            lido := trim(lido);
            if maiuscAnsi (copy (lido, 1, 6)) = 'NOMEIA' then
                begin
                     nomeia := true;
                     delete (lido, 1, 6);
                     lido := trim(lido);
            ...
            if lido = '' then
                ...
                if not extraiValor (lido, sep) then exit;
            if lido <> '' then exit;

            if not inicLista (lido, v) then exit;
                        ...
                        begin
                            ...
                            for col := 1 to colsSel do
                                begin
                                    ...
                                    linhaDaLista := linhaDaLista + prep[NumPrep].ColumnName(col-1);
                                end;
                            if nomeia then
                                listaScript[v].add(linhaDaLista);
                            primeiraLinha := false;
                        end;

                    linhaDaLista := '';

                    for col := 1 to prep[numPrep].ColumnCount do
                        begin
                            case tiposSel[col] of
                                0:  x := '';
                                1:  x := intToStr(prep[numPrep].ColumnInt(col-1));
                                2:  x := FloatToStr(prep[numPrep].ColumnDouble(col-1));
                                3:  x := prep[numPrep].ColumnText(col-1);
                                4:  ;  // Blob
                            end;
                            if col <> 1 then linhaDaLista := linhaDaLista + sep;
    ...
    if maiuscAnsi (copy (lido, 1, 5)) = 'EXIBE' then
        begin
            //SQL EXIBE "comando sql" separador
...
begin
    ...
    if cmd = 'SPRITE'    then executaComando := processaSprite (lido)
    else
    if cmd = 'SQL'       then executaComando := processaSql (lido)
    else
    if (cmd = 'AJUDA') or
