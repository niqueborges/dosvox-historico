...
{    Reaproveitado dE pypipe.pas, do programa pyvox,     }
{    do sistema dosvox.                                  }
{    Em 14/03/2014                                       }
{                                                        }
{--------------------------------------------------------}
...
interface
uses
    Windows, Classes, Sysutils, tlhelp32, dvwin, dvcrt;
...
begin
    ...
    Result := False;
    uex := UpperCase(ExeFileName);
    while ContinueLoop do
        begin
            if (UpperCase(ExtractFileName(FProcessEntry32.szExeFile)) = uex) or
               (UpperCase(FProcessEntry32.szExeFile) = uex) then
                    Result := True;
