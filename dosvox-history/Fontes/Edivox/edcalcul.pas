...
{    Efetuando cálculos
{
{    Autores: Marcelo Luis Pinheiro e Glauco Férius
{
...
uses
    ...
    edDesfaz,
    edVars, edUtil, edLinha, edMensag, edTela;
...
begin
    if tecla = F1 then
        ...
        begin
            gotoxy (1, 10);
            popupMenuCria (24, 10, 61, 26 - wherey, RED);
...
Begin
    ...
    If (x > 0) and (numero[x] = '(') Then {verifica se é subconta}
        begin
            ...
            s := sintAmbiente ('EDIVOX', 'PARTEDECIMAL', '2');
            val (s, i1, i);
            if i1 > 8 then
                begin
                    i1 := 8;
...
function seForData: boolean;
var
    s: string;
begin
    result := false;
    if (length(texto[posy]) <> 11) or
       (texto[posy][3] <> '/') or
       (texto[posy][6] <> '/') then exit;

    if (not tudoNumeral (copy (texto[posy], 1, 2))) or
       (not tudoNumeral (copy ( texto[posy], 4, 2))) or
       not tudoNumeral (copy (texto[posy], 7, 4)) then exit;

    try
        s := formatdatetime('AAAA, DD/MM/YYYY', strToDate(copy(texto[posy], 1, 10)));
...
label deNovo;
begin
    if seForData then exit; //Calcula o dia da semana.
...
deNovo:
    if not(tecla in [#0, ESC]) then gravarDesfazer;
    escreveTela;
