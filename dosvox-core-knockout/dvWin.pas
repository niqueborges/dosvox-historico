...
function  sintAmbiente (nomeSecao, nomeAmbiente: string): string; overload;
function  sintAmbiente (nomeSecao, nomeAmbiente, valorPadrao: string): string; overload;
...
var
    ...
    sintEcoarOpcao: boolean;
    sintFalarTudo: boolean;
...
var
    nomeDirSom      : string;
    falando         : boolean;
    divisorTempoBipsCleks: integer = 1;
...
function sintAmbienteArq (nomeSecao, nomeAmbiente, default, nomeArqIni: string): string;
var
    returnString: array[0..50000] of char; // Valor original 255
begin
    GetPrivateProfileString(pchar(nomeSecao), pchar(nomeAmbiente),
         pchar(default), returnString, 50000, pchar(nomeArqIni));
...
begin
    s := sintAmbienteArq (nomeSecao, nomeAmbiente, '', dosvoxIniDir + '\'+DOSVOXINIFILE);

    { se não achou no ambiente novo, tenta no ini de \windows }
    if (s = '') and (sintAceitaLegado) then
        s := sintAmbienteArq (nomeSecao, nomeAmbiente, '', DOSVOXINIFILE);
...
{--------------------------------------------------------}
{                  Retorna item do arquivo dosvox.ini gravando valor padrão se não existir
{--------------------------------------------------------}
...
var
    s: string;
begin
    s := trim(sintAmbiente (nomeSecao, nomeAmbiente));
    if s = '' then sintGravaAmbiente(nomeSecao, nomeAmbiente, valorPadrao)
    else valorPadrao := s;
...
var windir: string;
    pwindir: array [0..255] of char;
begin
    sintGravaAmbienteArq(secao, item, valor, dosvoxIniDir + '\'+DOSVOXINIFILE);
    ...
    if sintAceitaLegado then
        begin
            ...
            if fileExists(windir+'\'+DOSVOXINIFILE) then
                sintGravaAmbienteArq(secao, item, valor, DOSVOXINIFILE);
        end;
end;
...
procedure sintRemoveAmbiente (secao, item: string);
begin
    sintRemoveAmbienteArq(secao, item, sintDirAmbiente + '\'+DOSVOXINIFILE);
    if sintAceitaLegado then
        sintRemoveAmbienteArq(secao, item, DOSVOXINIFILE);
end;
...
begin
    ...
    geraCabWav (@clek, (TAMCLEK div divisorTempoBipsCleks), 11025, 8, 1);
    wavePlayMem (@clek);
    while sintFalando do waitMessage;
    compactWaves := salva;
end;
...
begin
    ...
    i := 0;
    while i < (TAMBIP div divisorTempoBipsCleks) do
        begin
            bip[i] := $80; inc (i);
            bip[i] := $70; inc (i);
            bip[i] := $60; inc (i);
    ...
    bip[(TAMBIP div divisorTempoBipsCleks)-1] := $80;

    geraCabWav (@bip, (TAMBIP div divisorTempoBipsCleks), 11025, 8, 1);
    wavePlayMem (@bip);
    while sintFalando do waitMessage;
end;
{$EndIf}
...
begin
    ...
    if length(s) > 1 then
        begin
            naofalar := sintambiente('DOSVOX','CARACTERESNAOFALAR', '');
            for i := 1 to length(naofalar) do
                begin
                    p := pos(naofalar[i],s);
                    while p > 0 do
                        begin
...
inicio:
    ...
    if c1 = #0 then
        begin
            c2 := readkey;
            if c2 in [#16..#18] then      { altGr q, w, e }
                c1 := readkey;
        end;
...
{--------------------------------------------------------}
    ...
    procedure delCarac (comDel, falaLetra: boolean);
    var campoPc: string;
        c: char;
    begin
        if not altera then
        ...
        if sintFalarTudo and comDel then sintSom ('_DEL');
...
processaC2:
                case c2 of
                    ESQ: if curx <= 1 then
                          begin
                              if cursorLegado then sintBip
                              else sintCarac(campo[curx]);   {   Fala letra se cursorlegado for false   }
                          end
                          else
                              begin
                                  curx := curx - 1;
                    ...
                    DIR: begin
                              if curx > length (campo) then
                                  ...
                                  begin
                                      if (cursorLegado = false) and (curx < length(campo)) then curx := curx + 1; {   False avança primeiro para falar depois   }
                                      if (not (upcase(campo [curx]) in ['A'..'Z', 'a'..'z'])) or
                                         (getKeyState (vk_Menu) >= 0) then
                                          sintCarac(campo [curx])
                                      else
                                          sintSom('_FON' + intToStr(ord(campo [curx])));
                                      if cursorLegado =true then curx := curx + 1;   {   Fala primeiro para avançar depois, padrão cursor legado estando em true   }
                                  end;
                         end;

                    HOME:  begin
                        curx := 1;
                        if not cursorLegado then sintCarac(campo[curx]);   { Fala letra no início da linha   }
                    end;
                    ...
                    TEND:  begin
                              ...
                              until (curx = 0) or (campo[curx] <> ' ');
                              curx := curx + 1;
                              if not cursorLegado then sintCarac(campo[curx]);
                          end;
                    ...
                    DEL: if (iniMarca > 0) and (fimMarca > 0) then
                             begin
                                  ...
                                  curx := iniMarca;
                                  if sintFalarTudo then sintSom ('_DEL');
                             ...
                             begin
                                 if not cursorLegado then
                                     begin
                                     delCarac (false, false);
                                     sintCarac(campo[curx]);   {   Fala o caractere após o caracter deletado   }
                                     end
                                 else
                                 begin
                                    delCarac(true, true);
...
begin
    CoInitialize (NIL);
    falando := upcase(sintAmbiente('DOSVOX', 'FALANDO', 'Sim')[1]) = 'S';
    sintFalaPont := true;
    sintNomeArq := '';
    falandoSapi := true;
    sintApagaAuto := true;
    ...
    sintFalaAcumulada := '';
    sintAceitaLegado := upperCase (copy (sintAmbiente ('DOSVOX', 'LEGADO'), 1, 1)) = 'S';
    sintEcoarOpcao := upcase(sintAmbiente('DOSVOX', 'ECOAROPCAO', 'Sim')[1]) = 'S';
    sintFalarTudo := upcase(sintAmbiente('DOSVOX', 'FALARTUDO', 'Sim')[1]) = 'S';
    if upcase(sintAmbiente('DOSVOX', 'BIPSCLECSCURTOS', 'NAO')[1]) = 'S' then divisorTempoBipsCleks := 2;

    for i := 0 to (TAMCLEK div divisorTempoBipsCleks) div 4 do
        clek[i] := $80 + random(60);
    for i := (TAMCLEK div divisorTempoBipsCleks) div 4 to (TAMCLEK div divisorTempoBipsCleks)-1 do
        clek[i] := $80;
