{--------------------------------------------------------}
{
{    Rotinas bбsicas para seleзгo de arquivos
{    Autor: Josй Antonio Borges
{    Em novembro/2001
{    Alterado em julho/2020
{
{--------------------------------------------------------}
...
var
    listArqPersistente: boolean;    // para poder consultar a lista apуs a seleзгo
                                    // neste caso, depois de consultar, chamar
                                    // obrigatoriamente a rotina liberaListArq

    teclaObtemNomeArq: char;        // sу para a funзгo obtemNomeArq
    brAntes: string;
...
const
    ...
    'E','ю','ю','O','O','O','U','U','Y','O','U','ю','ю','ю','ю','ю',
    'A','I','O','U','N','N','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю',
    'ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю','ю',
    'A','A','A','A','A','A','С','C','E','E','E','E','I','I','I','I',
    'ю','N','O','O','O','O','O','X','ю','U','U','U','U','Y','ю','ю',
    'A','A','A','A','A','A','С','C','E','E','E','E','I','I','I','I',
    'ю','N','O','O','O','O','O','X','ю','U','U','U','U','Y','ю','ю');
...
begin
    ...
    repeat
        ...
        if p = 0 then
            begin
                mascAux := mascMultipla;
                mascMultipla := '';
            end
        else
            begin
                mascAux := copy (mascMultipla, 1, p-1);
                delete (mascMultipla, 1, p);
            end;
        if mascAux = '' then continue
        else if uppercase(mascAux) = 'A' then mascSelecao := mascSelecao +'|a|б|а|г|в|д'
        else if uppercase(mascAux ) = 'E' then mascSelecao := mascSelecao + '|e|…|и|к|л'
        else if uppercase(mascAux ) = 'I' then mascSelecao := mascSelecao + '|i|н|м|о|п'
        else if uppercase(mascAux) = 'O' then mascSelecao := mascSelecao + '|o|у|т|ф|х|ц'
        else if uppercase(mascAux) = 'U' then mascSelecao:= mascSelecao + '|u|ъ|щ|ы|ь'
        else mascSelecao := mascSelecao + '|' + mascAux;
    until mascMultipla = '';
    if (mascSelecao <> '') and (mascSelecao[1] = '|') then delete (mascSelecao, 1, 1);
    mascMultipla := mascSelecao;
        ...
        if FindFirst(mascSelecao, atribArq, sr) = 0 then
            repeat
                if (((sr.Attr and atribArq) = atribArq) or
                   ((sr.Attr and (FaDirectory+FaVolumeId) = 0) and (atribArq = FaArchive))) and
...
procedure ordenaListArq (tipo: integer);
        ...
        var
            nome1, nome2: string;
        begin
            nome1 := semAcentos(PMySearchRec(item1)^.sr.FindData.cFileName);
            nome2 := semAcentos(PMySearchRec(item2)^.sr.FindData.cFileName);
