...
{
{    Adendos de Julio Tadeu C. da Silveira e Neno Albernaz
{
{    Original em agosto/2001
{
{    Versão atual em 03/06/2020
{
{--------------------------------------------------------}
...
uses
    ...
    dvcrt, dvwin, dvHora, dvmidi, dvAmplia, videovox;
...
Procedure opcoesOrdena (descendente: boolean);
...
var
    ...
    FolheiaTocandoArqSom: boolean = false;
...
begin
   ...
   yvis := 1;
   nopcoes := 0;
   setLength (opcoes, 1);    // o primeiro elemento será usado no shellsort
   new(opcoes[0]);
end;
...
jaFala:
        ...
        case c of
            ^Q: sintetiza (intToStr(qualOpcao) + ' de ' + intToStr (nOpcoes));
            ^C: begin
                PutClipBoard (pchar(opcoes[qualOpcao]^.nomeOpcao)); SintClek;
            end;
            ...
            if upcase(c) in ['0'..'9', 'A'..'Z', ^A..^Z, '+', '-', '*', '/'] then
                begin
                    ...
                    while (k <> qualOpcao ) and   (uppercase(semAcentos(copy(trim(opcoes[k]^.nomeOpcao), 1, 1))) <> upcase(c)) do
                        if k >= nOpcoes then
                            begin
                                k := 1;
                                sintClek; //Neno
                            ...
                            k := k + 1;
                    if uppercase(semAcentos(copy(trim(opcoes[k]^.nomeOpcao), 1, 1)))  = upcase(c) then
                        qualOpcao := k;
                end;
        end;

        if (nopcoes = 0) or (qualOpcao <= 0) then
            begin
                qualOpcao := 1;
                sintBip;
            end
        ...
        if (nopcoes <> 0) and (c <> ESC) and (c <> ENTER) then
...
Procedure opcoesOrdena (descendente: boolean);   // shellsort
Var
    i, j, step: Integer;
    tmp: POpcoes;
    tmps: string;
...
begin
    // nota: opcoes[0] contém o primeiro ou último elemento da ordenação
    if descendente then
        opcoes[0]^.nomeOpcao := #$ff+#$ff
    else
        opcoes[0]^.nomeOpcao := '';
        ...
        begin
            for i:=step to nopcoes do
                begin
                    ...
                    j := i;
                    while (j>=step) and (descendente xor
                          (maiuscAnsi(semAcentos(opcoes[j-step]^.nomeOpcao))>tmps)) Do
                        begin
...
begin
    ...
    if falando then
        ...
        if FolheiaTocandoArqSom and existeArqSom (oQueFalar) then
                sintSom (oQueFalar)
            else
                sintetiza (oQueFalar);
        end;
            ...
            if c1 <> #0 then
                 ...
                 begin
                      ...
                      case c2 of
                          ...
                          CIMA:              begin
                                                if apertouShift and (itemFolheado >= 0) and (itemFolheado < listaFol.count)  then
                                                    selecionado [itemFolheado] := false;
                                                itemFolheado := itemFolheado - 1;
                                             end;

                          BAIX, TAB:         begin
                                                if apertouShift and (itemFolheado >= 0) and (itemFolheado < listaFol.count)  and (not selecionado [itemFolheado]) then
                                                    begin
                                                        sintBip;
                                                        selecionado [itemFolheado] := true;
                                                    end;
                                                itemFolheado := itemFolheado + 1;
                                                if apertouShift and (itemFolheado >= 0) and (itemFolheado < listaFol.count)  then
                                                    selecionado [itemFolheado] := true;
                          ...
                          HOME, CTLPGUP:    begin
                                                if apertouShift then
                                                    for i := itemFolheado downto 0 do
                                                        if (itemFolheado >= 0) and (itemFolheado < listaFol.count)  then
                                                            selecionado [i] := false;
                                                sintBip;
                                                itemFolheado := 0;
                                            end;

                          CTLHOME:          if apertouShift then
                                            begin
                                                for i := itemFolheado downto 0 do
                                                    if (itemFolheado >= 0) and (itemFolheado < listaFol.count)  then
                                                        selecionado [i] := true;
                          ...
                          TEND, CTLPGDN:    begin
                                                if apertouShift then
                                                    for i := itemFolheado to listaFol.count-1 do
                                                        if (itemFolheado >= 0) and (itemFolheado < listaFol.count)  then
                                                            selecionado [i] := true;
                                                sintBip;
