...
{                                                       }
{       Alterado por Patrick Barboza em Abril/2024 e    }
{       em Julho/2024                                   }
{                                                       }
{*******************************************************}
...
{   Legacy cursor definition   }

var
    cursorLegado: boolean = true;

{ Dosvox configuration files }

const
    DOSVOXINI = 'Dosvox.ini';   //Arquivo padrão do sistema
    DVINSTALINI = 'DV-Instal.ini';   //Arquivo de configuração do instalador falado do Dosvox

var
    DOSVOXINIFILE: string = DOSVOXINI;   //Padrão Dosvox.imni até nova alteração

procedure defineArqIni(arq: string);
...
{ Braille area }
  ...
  procedure refreshBrlCliente (yy: integer);  forward;   //***Linhas Braille
...
begin
  if cursorLegado then   //Desenha o cursor padrão Dosvox
  begin
  ...
  else   //Novo cursor, baseado no WIndows
  begin
    CreateCaret(CrtWindow, 0, 2, -CharSize.Y);
    SetCaretPos((currentWindow.left+Cursor.X) * CharSize.X,
      (currentWindow.top+Cursor.Y) * CharSize.Y + CharAscent+2);
end;
  ShowCaret(CrtWindow);
end;
...
begin
  ...
  if linhaBraillePresente then   //***Linhas Braille
      refreshBrlCliente (Cursor.Y);
end;
...
begin
  ...
  begin
    ...
    if (not painting) and linhaBraillePresente then   //***Linhas Braille
        refreshBrlCliente(Y);
...
{ Braille cliente refresh }

procedure refreshBrlCliente (yy: integer);   //***Linhas Braille
begin
      with currentWindow do
setWindowTitle(copy (StrPas (ScreenPtr(0, yy+Top)), 1, ScreenSize.X));
    if linhaBraillePresente then
      with currentWindow do
        writeBrlServer (copy (StrPas (ScreenPtr(0, yy+Top)), 1, ScreenSize.X));
end;
...
procedure WindowTimer (id: integer);
begin
    timerIsSet := true;
    timerId := id;
end;
...
begin
  ...
  case Message of
    ...
    if hasBrailleKbdCallback and (Message = uint(brailleKbdMsg)) then //***Linhas Braille
      begin
...
{   choose Dosvox ini file   }

procedure defineArqIni(arq: string);   //Patrick
begin
    if (arq <> DOSVOXINI) and (arq <> DVINSTALINI) then
    begin
        DOSVOXINIFILE := DOSVOXINI;   //Dosvox.ini caso venha outro nome diferente dos esperados
    end
    else
    begin
        DOSVOXINIFILE := arq;
...
begin
    ...
    if not fileExists(dir+'\'+DOSVOXINIFILE) then   // Roaming não existe ainda...
        begin
...
const
    ...
    altLetra734 = 28;
begin
    dosvoxIniFilename := dosvoxIniDir + '\'+DOSVOXINIFILE;
    ...
    n := GetPrivateProfileString('VIDEO', 'CURSORLEGADO', 'S', resp, 80, pchar(dosvoxIniFileName));   //Patrick
    if (n <> 0) and (upcase(resp[0]) = 'N') then cursorLegado := false;
...
begin
  ...
  begin
    ...
    dvBrlCliente.connectBrlServer;   //***Linhas Braille
    //Patrick
    //O cliente se encarrega de indicar se conexão foi bem sucedida
    //Esta indicação é feita definindo para true
    //a variável linhaBraillePresente
  end;
end;
...
begin
  ...
  brailleKbdMsg := RegisterWindowMessage('dvLinBrl');   //***Linhas Braille
